...
```bash
# Determine target branch: use existing PR base if available, otherwise fall back to env var or main
TARGET=$(gh pr view --json baseRefName -q '.baseRefName' 2>/dev/null || echo "${CONDUCTOR_TARGET_BRANCH:-main}")
...
**IMPORTANT: Use `subagent_type: "general-purpose"` with `model: "opus"` for ALL subagents.**
...
Review this PR for SIMPLICITY. Determine the target branch with: `TARGET=$(gh pr view --json baseRefName -q '.baseRefName' 2>/dev/null || echo "${CONDUCTOR_TARGET_BRANCH:-main}")` then `git fetch origin "$TARGET"` and run `git diff "origin/$TARGET...HEAD"` to see changes.
...
Review this PR for CODING PATTERNS and BEST PRACTICES. Determine the target branch with: `TARGET=$(gh pr view --json baseRefName -q '.baseRefName' 2>/dev/null || echo "${CONDUCTOR_TARGET_BRANCH:-main}")` then `git fetch origin "$TARGET"` and run `git diff "origin/$TARGET...HEAD"`.
...
Review this PR for SECURITY. Determine the target branch with: `TARGET=$(gh pr view --json baseRefName -q '.baseRefName' 2>/dev/null || echo "${CONDUCTOR_TARGET_BRANCH:-main}")` then `git fetch origin "$TARGET"` and run `git diff "origin/$TARGET...HEAD"`.
...
Review this PR for PRODUCTION READINESS. Determine the target branch with: `TARGET=$(gh pr view --json baseRefName -q '.baseRefName' 2>/dev/null || echo "${CONDUCTOR_TARGET_BRANCH:-main}")` then `git fetch origin "$TARGET"` and run `git diff "origin/$TARGET...HEAD"`.
...
Review this PR's TESTS. Determine the target branch with: `TARGET=$(gh pr view --json baseRefName -q '.baseRefName' 2>/dev/null || echo "${CONDUCTOR_TARGET_BRANCH:-main}")` then `git fetch origin "$TARGET"` and run `git diff "origin/$TARGET...HEAD"` to see the changes, then examine
...
- Use non-opus models for subagents (these reviews require deep reasoning)
...
- `pull-request` - Creates PR and automatically invokes this review
