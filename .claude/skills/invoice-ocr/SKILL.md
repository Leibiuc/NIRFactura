---
name: invoice-ocr
description: Cost-effective invoice transcription via Claude vision. Apply when implementing or reviewing lib/ocr.ts to ensure lowest token cost per extraction.
license: MIT
metadata:
  author: NIRFactura
  version: '1.0'
...
Always use `claude-sonnet-4-6`. Do not use Opus for structured extraction — it costs 67% more per token for no benefit on this task.
